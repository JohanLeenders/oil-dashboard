    ...
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-sm text-gray-500 mb-1">Onderdelen geanalyseerd</div>
          <div className="text-3xl font-bold text-gray-900">{totalParts}</div>
          <div className="text-sm text-gray-500 mt-1">Laatste 12 maanden</div>
        </div>

        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-sm text-gray-500 mb-1">Marge stijgend</div>
          <div className="text-3xl font-bold text-green-600">{partsWithUpTrend}</div>
          <div className="text-sm text-gray-500 mt-1">onderdelen</div>
        </div>

        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-sm text-gray-500 mb-1">Marge dalend</div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="text-sm text-gray-500 mt-1">{customersWithUpTrend} met stijgende marge</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow mb-8">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Tijdlijn per Onderdeel</h2>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {partSummaries.map((summary) => (
                <div key={summary.part_code} className="border border-gray-200 rounded-lg p-4">
            ...
            {/* Part Timeline Table */}
            <div className="mt-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Detailweergave per Periode</h3>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Rendement</th>
                      <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Marge %</th>
                      <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Volume (kg)</th>
                      <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Omzet</th>
                      <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">DSI</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {Array.from(partTrendsByPart.entries()).flatMap(([partCode, rows]) =>
                      rows.slice(0, 6).map((row, idx) => (
                        <tr key={`${partCode}-${row.period_start}`} className={idx === 0 ? 'bg-blue-50' : 'hover:bg-gray-50'}>
                          ...
                          <td className="px-3 py-2 text-right">
                            <span className={row.avg_margin_pct !== null && row.avg_margin_pct < 0 ? 'text-red-600' : 'text-gray-900'}>
      ...
      <div className="bg-white rounded-lg shadow mb-8">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Tijdlijn per Klant</h2>
          <p className="text-sm text-gray-500 mt-1">
        ...
        {customerSummaries.length === 0 ? (
          ...
          <div className="p-6">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Volume</th>
                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Marge</th>
                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Karkasbalans</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Recent Marge</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Vorige Marge</th>
                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Periodes</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {customerSummaries.map((summary) => (
                    <tr key={summary.customer_id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <div className="font-medium text-gray-900">{summary.customer_name}</div>
                      </td>
            ...
            <div className="mt-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Detailweergave per Klant</h3>
                  ...
                  return (
                    <div key={customerId} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <div className="font-medium text-gray-900">{firstRow.customer_name}</div>
                          <div className="text-sm text-gray-500">{firstRow.customer_code}</div>
                        </div>
                      </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead className="bg-gray-50">
                            <tr>
                              <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Periode</th>
                              <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">Volume (kg)</th>
                              <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">Omzet</th>
                              <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">Marge</th>
                              <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">Alignment</th>
                              <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">Volume wijz.</th>
                              <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">Marge wijz.</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100">
                            {rows.slice(0, 6).map((row, idx) => (
                              <tr key={row.period_start} className={idx === 0 ? 'bg-blue-50' : ''}>
                                ...
                                <td className="px-3 py-2 text-right">
                                  <span className={row.margin_pct !== null && row.margin_pct < 0 ? 'text-red-600' : 'text-gray-900'}>
      ...
      <div className="bg-white rounded-lg shadow mb-8">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Annotaties</h2>
          <p className="text-sm text-gray-500 mt-1">
        ...
        {batchHistory.length === 0 ? (
          ...
          <div className="p-6">
            {/* Season Summary */}
            <div className="mb-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Seizoensoverzicht</h3>
                ...
                {Array.from(batchesBySeason.entries()).map(([season, batches]) => {
                  ...
                  return (
                    <div key={season} className="border border-gray-200 rounded-lg p-4">
                      <div className="font-medium text-gray-900 mb-2">{getSeasonLabel(season)}</div>
                      <div className="text-sm text-gray-500">{batches.length} batches</div>
                      <div className="mt-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-gray-500">Gem. rendement:</span>
                          <span className="text-gray-900">{avgYield > 0 ? `${avgYield.toFixed(1)}%` : '-'}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Gem. marge:</span>
                          <span className={avgMargin < 0 ? 'text-red-600' : 'text-gray-900'}>
            ...
            {/* Recent Batches */}
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">Recente Batches</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Batch</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Slachtdatum</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Seizoen</th>
                      <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Vogels</th>
                      ...
                      <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Volledigheid</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {batchHistory.map((batch) => (
                      <tr key={batch.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 font-medium text-gray-900">{batch.batch_ref}</td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={batch.total_margin_pct !== null && batch.total_margin_pct < 0 ? 'text-red-600' : 'text-gray-900'}>
